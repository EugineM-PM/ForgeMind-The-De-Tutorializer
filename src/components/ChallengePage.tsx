...
import { getDemoPresetsForConcept } from '../data/demoResponses';
...
export const ChallengePage: React.FC<ChallengePageProps> = ({
  ...
  // Available demo presets for presentation
  const demoPresets = getDemoPresetsForConcept(concept.id);
    ...
    <div id="challenge-page" className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-4">
        <button
          ...
          className="inline-flex items-center space-x-2 rounded-xl border border-slate-200 bg-white px-3.5 py-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors shadow-sm"
        >
        ...
        <div className="flex items-center space-x-3">
          ...
          {challenge && (
            <span className="rounded-full border border-slate-200 bg-slate-100 px-3 py-0.5 text-xs font-medium text-slate-600">
              Source: <strong className="text-slate-800 font-semibold">{challenge.sourceType}</strong>
            </span>
          )}

          <span className="text-xs text-slate-500 hidden md:inline">
            Domain: <strong className="text-slate-700 font-medium">{concept.domain}</strong>
          </span>
        </div>
      </div>
      ...
      {/* Loading State */}
      {isLoading && (
        <div className="mt-12 flex flex-col items-center justify-center rounded-3xl border border-slate-200/90 bg-white p-12 text-center shadow-card">
          <div className="relative mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-50 text-[#006BFF] border border-blue-200">
            <RefreshCw className="h-7 w-7 animate-spin text-[#006BFF]" />
            <Sparkles className="absolute -top-1.5 -right-1.5 h-4 w-4 text-[#006BFF] animate-pulse" />
          </div>
          ...
          <p className="mt-2 max-w-md text-sm text-slate-500 leading-relaxed">
            Generating an unfamiliar workplace scenario for <strong className="text-slate-900 font-semibold">{concept.name}</strong> to measure independent application.
          </p>

          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4 text-xs text-slate-400 font-medium">
            <span className="flex items-center space-x-1.5">
              <span className="h-2 w-2 rounded-full bg-[#006BFF] animate-ping" />
              <span className="text-[#006BFF]">Applying trade-off constraints</span>
            </span>
      ...
      {!isLoading && generationError && !challenge && (
        <div className="mt-12 rounded-3xl border border-rose-200 bg-rose-50/40 p-8 text-center sm:p-12 shadow-card">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-rose-100 text-rose-600 border border-rose-200">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            <button
              ...
              className="inline-flex items-center space-x-2 rounded-xl bg-[#006BFF] px-4 py-2 text-xs font-semibold text-white hover:bg-[#005CE6] transition-colors shadow-sm"
            >
            ...
            {getCuratedNovelChallenge(concept.id) && (
              <button
                ...
                className="inline-flex items-center space-x-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm"
              >
        ...
        <div className="mt-6">
                ...
                <div className="mt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-zinc-800/80 pt-5">
                  ...
                  <div className="flex items-center space-x-3">
                    {concept.id === 'rice-prioritization' && (
                      <button
                        type="button"
                        id="demo-pitch-confidence-5-btn"
                        onClick={() => {
                          setConfidenceBeforeAttempt(5);
                          handleStartIndependentAttempt();
                        }}
                        className="inline-flex items-center space-x-1.5 rounded-lg border border-amber-500/40 bg-amber-500/10 hover:bg-amber-500/20 px-4 py-3 text-xs font-semibold text-amber-300 transition-all"
                        title="Primary Pitch Path: Set 5/5 Confidence & Begin Independent Attempt"
                      >
                        <Sparkles className="h-3.5 w-3.5 text-amber-400" />
                        <span>Pitch Demo: 5/5 Confidence & Begin</span>
                      </button>
                    )}
          ...
          {stage === 'attempt' && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/80 px-4 py-3 text-xs text-emerald-800">
                <div className="flex items-center space-x-2.5">
                  <Lock className="h-4 w-4 flex-shrink-0 text-emerald-600" />
                  <div>
                    <span className="font-bold">Independent Attempt Active:</span> Zero-reference mode enforced. Original examples, reference solutions, evaluation rubrics, and hint ladders are concealed.
                  </div>
                </div>

                <div className="flex items-center space-x-3 text-slate-500 text-xs">
                  <span>Confidence: <strong className="text-slate-900 font-bold">{confidenceBeforeAttempt}/5</strong></span>
                  {draftSavedTimestamp && (
                    <span className="text-slate-400">Draft: {draftSavedTimestamp}</span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                ...
                <div className="lg:col-span-5 space-y-4">
                  <div className="rounded-3xl border border-slate-200/90 bg-white p-6 shadow-card space-y-5">
                    ...
                    <div className="rounded-2xl border border-blue-200 bg-blue-50/60 p-4">
                      <div className="flex items-center space-x-1.5 text-xs font-bold uppercase tracking-wider text-[#006BFF] mb-1">
                        <CheckCircle2 className="h-4 w-4" />
                        <span>Mandate</span>
                      </div>
                    ...
                    <div>
                      ...
                      <p className="mt-1.5 text-xs text-slate-600 leading-relaxed max-h-48 overflow-y-auto pr-1">
                        {challenge.scenario}
                      </p>
                    </div>
                    ...
                    {challenge.contextData && (
                      <div className="rounded-2xl border border-slate-200 bg-slate-900 p-4">
                        <span className="text-xs font-semibold uppercase tracking-wider text-blue-400 block mb-1.5">
                          Parameters / Telemetry
                        </span>
                        <pre className="text-xs font-mono text-slate-200 whitespace-pre-wrap max-h-40 overflow-y-auto pr-1">
                          {challenge.contextData}
                        </pre>
                      </div>
                    )}
                      ...
                      <ul className="mt-2 space-y-1.5">
                        {challenge.constraints.map((c, i) => (
                          <li key={i} className="flex items-start space-x-2 text-xs text-slate-700">
                            <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#006BFF] flex-shrink-0" />
                            <span>{c}</span>
                          </li>
                        ))}
                      </ul>
                ...
                <div className="lg:col-span-7 space-y-4">
                  <div className="rounded-3xl border border-slate-200/90 bg-white p-6 shadow-card flex flex-col h-full">
                    {/* Buildathon Demo Assistant Presets (if available) */}
                    {demoPresets.length > 0 && (
                      <div className="mb-4 rounded-2xl border border-blue-200 bg-blue-50/50 p-3.5">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div className="flex items-center space-x-2 text-xs text-slate-700 font-semibold">
                            <Sparkles className="h-3.5 w-3.5 text-[#006BFF] flex-shrink-0" />
                            <span>Buildathon Demo Assistant:</span>
                          </div>
                          <div className="flex flex-wrap items-center gap-2">
                            {demoPresets.map((preset) => (
                              <button
                                ...
                                className={`inline-flex items-center space-x-1.5 rounded-xl px-3 py-1 text-xs font-semibold border transition-all ${
                                  preset.expectedVerdict === 'CORRECT'
                                    ? 'border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100'
                                    : 'border-blue-200 bg-blue-100/50 text-[#006BFF] hover:bg-blue-100'
                                }`}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3.5">
                      ...
                      {previewTab === 'write' && (
                        <div className="flex items-center space-x-1 bg-slate-100 border border-slate-200 rounded-xl p-1 text-slate-600">
                          <button
                            type="button"
                            onClick={() => insertFormatting('**', '**')}
                            title="Bold (**text**)"
                            className="p-1 hover:text-slate-900 hover:bg-white rounded-lg transition-colors"
                          >
                      ...
                      {previewTab === 'write' ? (
                        <div className="relative">
                          <textarea
                            ...
                            className={`w-full rounded-2xl border bg-slate-50/60 p-4 text-xs leading-relaxed text-slate-900 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 transition-all ${
                              response.length >= MAX_RESPONSE_LENGTH
                                ? 'border-rose-300 focus:border-rose-500 focus:ring-rose-500/20'
                                : 'border-slate-200 focus:border-[#006BFF] focus:ring-[#006BFF]/20'
                            } ${editorMode === 'code' ? 'font-mono' : 'font-sans'}`}
                          />
                        </div>
                      ) : (
                        <div className="min-h-[380px] rounded-2xl border border-slate-200 bg-slate-50/50 p-4 text-xs text-slate-800 whitespace-pre-wrap overflow-y-auto">
                    ...
                    <div className="mt-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-slate-100 pt-4">
                      <div className="flex items-center space-x-2">
                        <button
                          ...
                          className="inline-flex items-center space-x-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm"
                        >
                      ...
                      <button
                        ...
                        className={`inline-flex items-center justify-center space-x-2 rounded-xl px-6 py-2.5 text-xs font-semibold transition-all shadow-button ${
                          response.trim().length === 0 || isSubmitting
                            ? 'bg-slate-200 text-slate-400 cursor-not-allowed border border-slate-300'
                            : 'bg-[#006BFF] text-white hover:bg-[#005CE6] active:scale-[0.99]'
                        }`}
                      >
                        {isSubmitting ? (
                          <>
          ...
          {stage === 'submitted' && submittedAttempt && (
            <div className="space-y-6">
              ...
              >
                ...
                <p className="mt-4 text-xs text-slate-600 sm:text-sm leading-relaxed max-w-2xl">
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 text-xs">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Attempt ID</span>
                    <span className="font-semibold text-slate-800 truncate block mt-0.5">{submittedAttempt.attempt_id}</span>
                  </div>

                  <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Pre-Attempt Confidence</span>
                    <span className="font-bold text-[#006BFF] block mt-0.5">{submittedAttempt.confidence_before_attempt} / 5</span>
                  </div>

                  <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Response Length</span>
                    <span className="font-semibold text-slate-800 block mt-0.5">{submittedAttempt.response.length.toLocaleString()} chars</span>
                  </div>

                  <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Source Type</span>
                    <span className="font-semibold text-slate-800 block mt-0.5">{submittedAttempt.source_type}</span>
                  </div>
                </div>

                {/* Unedited Response Review */}
                <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-900 p-5 shadow-sm">
                  ...
                  <pre className="overflow-x-auto text-xs font-mono text-slate-200 whitespace-pre-wrap leading-relaxed max-h-72">
                  ...
                  {isEvaluating && (
                    <div className="rounded-2xl border border-blue-200 bg-blue-50/50 p-6 text-center">
                      <div className="flex justify-center">
                        <RefreshCw className="h-6 w-6 text-[#006BFF] animate-spin" />
                      </div>
                    ...
                    <div className="rounded-2xl border border-rose-200 bg-rose-50/60 p-5">
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          <h4 className="text-xs font-bold text-rose-900">Evaluation Interrupted</h4>
                          <p className="text-xs text-slate-600 mt-1">{evaluationError}</p>
                          <button
                            type="button"
                            onClick={handleRerunEvaluation}
                            className="mt-3 inline-flex items-center space-x-1.5 rounded-xl bg-white border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm"
                          >
                            <RefreshCw className="h-3.5 w-3.5" />
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-4">
                        <div className="flex items-center space-x-3.5">
                          ...
                          {evaluationResult.verdict === 'WRONG_APPROACH' && (
                            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-rose-200 bg-rose-50 text-rose-600">
                              <AlertOctagon className="h-5 w-5" />
                            </div>
                          )}
                          {evaluationResult.verdict === 'NEEDS_CLARIFICATION' && (
                            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-purple-200 bg-purple-50 text-purple-600">
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="rounded-2xl border border-slate-200 bg-white px-3.5 py-2 text-right shadow-sm">
                          ...
                          <button
                            ...
                            className="rounded-xl border border-slate-200 bg-white p-2.5 text-slate-500 hover:text-slate-900 hover:bg-slate-50 transition-colors shadow-sm"
                          >
                            <RefreshCw className="h-3.5 w-3.5" />
                          </button>
                        </div>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-2 text-slate-900">
                            <Quote className="h-3.5 w-3.5 text-[#006BFF]" />
                            <h4 className="text-xs font-bold uppercase tracking-wider">
                        ...
                        {evaluationResult.evidence.length > 0 ? (
                          <div className="space-y-2">
                            {evaluationResult.evidence.map((ev, idx) => (
                              <div
                                key={idx}
                                className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 text-xs font-mono text-slate-700 leading-relaxed"
                              >
                                {ev}
                              </div>
                            ))}
                ...
                <div className="mt-6 flex flex-wrap items-center gap-3">
                  <button
                    ...
                    className="inline-flex items-center space-x-2 rounded-xl bg-[#006BFF] px-5 py-2.5 text-xs font-semibold text-white hover:bg-[#005CE6] transition-colors shadow-button"
                  >
                  ...
                  <button
                    id="retry-with-hints-action-btn"
                    onClick={handleRetryWithHints}
                    className="inline-flex items-center space-x-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs font-semibold text-[#006BFF] hover:bg-blue-100 transition-colors shadow-sm"
                  >
                    <RotateCcw className="h-3.5 w-3.5" />
                    <span>Retry Attempt with Learned Insights</span>
                  </button>

                  <button
                    id="make-another-attempt-btn"
                    onClick={handleMakeAnotherAttempt}
                    className="inline-flex items-center space-x-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm"
                  >
                    <RotateCcw className="h-3.5 w-3.5 text-slate-500" />
                    <span>Make Fresh Attempt</span>
                  </button>

                  <button
                    onClick={handleRegenerateChallenge}
                    className="inline-flex items-center space-x-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-600 hover:bg-slate-50 transition-colors shadow-sm"
                  >
